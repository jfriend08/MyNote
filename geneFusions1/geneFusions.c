...
int main (int argc, char *argv[])
{
  ...
  int mrfLines;
    ...
    int a,k, tempIntrasize;  //peter
    char  r1[100]={0},r2[100]={0};   //s[80],   r1[50201],r2[50201];   //s[80],
    int * v,w,x,y,z, p;  //v[50201],w[50201],x[50201],y[50201],z[50201], p;
//    Interval *v;
    ...
    for (i = 0; i < arrayMax (currMrfRead1->blocks); i++) {
      ...
      for (j = 0; j < arrayMax (currMrfRead2->blocks); j++) {
        ...
        if (arrayMax (intervals1) == 1 && arrayMax (intervals2) == 1) {
          ...
          if(a == 40000000 || a == 40000001 || a == 40000002 || a == 40000003 || a == 40000004 || a == 40000005 || a == 45000000 || a == 50000000 || a == 55000000 || a == 56000000 || a == 57000000 || a == 58000000 || a == 59000000 || a == 60000000){
              warn("inters element:%d",arrayMax (inters));
              warn("intras: %d",arrayMax (intras));
          }
      ...
      while (a>50000)
      {
          ...
          warn("hello~");
//          tempIntrasize= arrayMax(intras);
          stringPrintf(buffer,"%s_intra_%d.txt",argv[1],k);
          warn("%s",string(buffer));
...
///////////////peter: read the splited intra files//////////////////////////////
    if((fp=fopen("AS182_1_chim_Align_merged.mrfSort_00_100000lines_intra_0.txt","r")) == NULL) {
...
//        warn("%d", currIntra->transcript);
    ...
    warn("number of new intra:%d ",arrayMax(intras));
/////////////peter/////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////superIntras////////////////////////////////////////////////////////////////////////////////
  ...
  arraySort (intras,(ARRAYORDERF)sortIntrasByTranscript);
    warn ("I am outside the loop!");

  intraOffsets = arrayCreate (1000000,int);
  superIntras = arrayCreate (100000,SuperIntra);
  i = 0;
  while (i < arrayMax (intras)) {      //this is for calculating intraOffstes?
    currIntra = arrp (intras,i,Intra);
    currSuperIntra = arrayp (superIntras,arrayMax (superIntras),SuperIntra);   //I can test arrayMax (superIntras) to see how it operate
