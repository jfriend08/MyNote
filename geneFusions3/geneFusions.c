...
//      warn("genomic: %d, transcript: %d, chromosome: %s", currCoordinate->genomic, currCoordinate->transcript, currCoordinate->chromosome);//peter
  }
  return coordinates;
...
int main (int argc, char *argv[])
{
  ...
  int mrfLines;
    ...
    int * v,w,x,y,z, p;
    SuperIntra testSuperIntra; //peter
    int index;
  ...
  while (currMrfEntry = mrf_nextEntry ()) {
    ...
    warn("number of remaining intra:%d ",arrayMax(intras));
    mrf_deInit ();
    ...
    for (k=0 ; k<numTxt; k++)
    {
        ...
        while (i < arrayMax (intras)) {
            ...
            testSuperIntra.transcript = currIntra->transcript;  //peter
            arrayFindInsert(superIntras, &testSuperIntra, &index, (ARRAYORDERF)sortSuperIntras); //peter
            currSuperIntra = arrayp (superIntras,index,SuperIntra);
            
            //currSuperIntra = arrayp (superIntras,arrayMax (superIntras),SuperIntra);
            //currSuperIntra->transcript = currIntra->transcript;
//            warn("i is: %d, index: %d, arrayMax(superIntra): %d, arrayMax(currSuperIntra->intras):%d",i, index, arrayMax(superIntras), arrayMax(currSuperIntra->intras));

            if (index == arrayMax (superIntras)-1){  //peter
                warn("hi!");
                currSuperIntra->transcript = currIntra->transcript;
                currSuperIntra->intras = arrayCreate (100,Intra*);  //peter
            }//peter
            warn("i is: %d, index: %d, arrayMax(superIntra): %d",i, index, arrayMax(superIntras));
//            currSuperIntra->intras = arrayCreate (100,Intra*);
//            warn("hi");
            warn(" the Max(currSuperIntra->intras): %d", arrayMax(currSuperIntra->intras));
            array (currSuperIntra->intras,arrayMax(currSuperIntra->intras),Intra*) = currIntra;
            warn("after");
            //if (i==0){
                //warn("i is: %d, the number of intra reads is: %d", i,arrayMax(currSuperIntra->intras));  //peter
            //}
            
            j = i + 1;
            while (j < arrayMax (intras)) {
                nextIntra = arrp (intras,j,Intra);
                warn("inside next Intra loop");
            ...
            i = j; warn("i and j is: %d", i);
                warn("i is: %d, index is: %d, the number of intra reads is: %d", i,index, arrayMax(currSuperIntra->intras));  //peter
        ...
        fclose(fp);
        warn("number of lines from txt: %d ",n);
